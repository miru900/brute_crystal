...
def mod_cell_integer(key, settings):
    ...
    ewal_mat = pcew.get_Ewald(settings[key]["grid"], settings[key]["cell_size"])
    ...
    # ewal_mat = pcew.ewal_mat_gen(pos_data2, settings[key]["cell_size"] * 1e-10)

    print(f"open_grid ~ generating matrix took {time.time() - start} seconds.")
    
    # integer_program.py
    # position, op_energy = ip.integer(chem, ion_count, elec_mat)
    position, op_energy = ip.integer(chem, ion_count, ewal_mat)
