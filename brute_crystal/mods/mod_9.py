...
import cal_fermion.fermion_qubo as frqb
...
def fermion_simulator(key, settings):
    ...
    if settings[key]["cons"]:
        energy, bits = frqb.simulator_init_cons(ewal_mat.dist, chem, ion_count, charge, settings[key]["cons"])
    else:
        pass
    
    obj_value = energy[0]
    print(f"Simulating Fermion Circuit has been finished, time taken from starting is {time.time() - start} seconds.")

    # processing part 1, Fermion Distribution Result
    try:
        with open(f"../results/{settings[key]['Symbol']}_fermion/distribution.txt", "w") as f:
            for (ene, bit) in zip(energy, bits):
                f.write("array : " + str(bit) + " | " + "value : " + str(ene) + "\n")
    except:
        print(f"../results/{settings[key]['Symbol']}_fermion/distribution.txt, the directory does not exist!")

    # processing part 2, bit to answers
    if settings[key]["cons"]:
        position = frqb.decode_answer_cons(bits, settings[key]["cons"], chem); print(position)
    else:
        pass
    ...
    if angle_if:
        ...
        for i in range(solcount):
            ...
            output_dir = f"../results/{settings[key]['Symbol']}_fermion" ; os.makedirs(output_dir, exist_ok = True) ; file_path = os.path.join(output_dir, f"{i}_fermion.cif")
            write(file_path, atoms)
            write(file_path.replace(".cif", ".vasp"), atoms, format='vasp')

            with open(f"../results/{settings[key]['Symbol']}_fermion/energy.txt", "w") as f:
        ...
        output_dir = f"../results/{settings[key]['Symbol']}_fermion" ; os.makedirs(output_dir, exist_ok = True) ; file_path = os.path.join(output_dir, f"{settings[key]['Symbol']}_fermion.cif")
        write(file_path, atoms)
        write(file_path.replace(".cif", ".vasp"), atoms, format='vasp')
    ...
    with open(f"../results/{settings[key]['Symbol']}_fermion/time.txt", "w") as f:
