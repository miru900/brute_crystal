...
def mod_cell_int_periodic2(key, settings):
    ...
    ewal_mat = pcew2.Ewald(pos_data2, settings[key]["cell_size"])
    ewal_mat.vec_gen()  ;  ewal_mat.ewal_mat_gen()  ;   print(ewal_mat.dist)
    buck_info = [settings[key]["grid"], settings[key]["cell_size"], phase]

    print(f"open_grid ~ generating matrix took {time.time() - start} seconds.")
    
    # integer_program.py
    position, op_energy = ip.integer(chem, ion_count, ewal_mat.dist, buck_info)
