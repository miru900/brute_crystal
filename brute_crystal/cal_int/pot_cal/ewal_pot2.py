...
scale = (elec_c ** 2) * 1e-10 / (4 * math.pi * ep0 * eevee) # scaling factor : e ** 2 / 4 * π * ε_0
...
class Ewald:
    ...
    def ewal_mat_gen(self, alpha = -1):
        ...
        pos = self.position @ self.vecs  ;  self.dist = np.zeros((N, N))  ;  shifts = np.zeros(((2 * realDepth + 1) ** 3 - 1, 3))
